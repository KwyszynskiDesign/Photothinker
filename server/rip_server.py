...
PANTONE_RECIPES = {
    "pantone 221 c": (0,100,53,36), "pantone 221 u": (0,100,53,36), "pantone 221": (0,100,53,36),
...
def get_recipe(name):
    ...
    # 1. Sprawdzenie dokładnego dopasowania słownikowego
    for key, recipe in PANTONE_RECIPES.items():
        if key == lower or key in lower or lower in key:
            return recipe
            
    # 2. Sprawdzenie dopasowania po samym numerze cyfrowym
    ...
    # 3. Inteligentne heurystyki nazewnicze (zabezpieczenie przed zielenią)
    if any(x in lower for x in ["red", "rubine", "magenta", "rhodamine", "warm red", "rose", "pink"]):
        return (0, 95, 90, 0)
    if any(x in lower for x in ["blue", "reflex", "cyan", "navy"]):
        return (100, 80, 0, 0)
    if any(x in lower for x in ["yellow", "gold"]):
        return (0, 15, 100, 0)
    if "orange" in lower:
        return (0, 65, 100, 0)
    if "green" in lower:
        return (90, 0, 90, 0)
    if any(x in lower for x in ["violet", "purple"]):
        return (75, 90, 0, 0)

    # 4. Fallback końcowy dla warstw technicznych lub nieznanych spotów
    if classify_color(name) == "tech":
        return (0, 100, 0, 0)
    return (0, 0, 0, 40)  # Neutralny szary zamiast mylącej jaskrawej zieleni
...
// Modifikacja ładowania obrazu w celu zapobieżenia miganiu i ukrywania nieaktywnych warstw info
